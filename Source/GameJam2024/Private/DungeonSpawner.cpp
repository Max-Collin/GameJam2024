...
// Sets default values
ADungeonSpawner::ADungeonSpawner(): BranchingThreshold(0), BranchingChance(0), Merging(false), Branching(false),
                                        MaxLoops(0), Scale(0),
                                        MaxExtents()
{
	...
	Tags.Add("Generator");
	SpawnerComponent = CreateDefaultSubobject<UItemSpawner>(TEXT("SpawnerComponent"));
}
...
void ADungeonSpawner::GenerateMap()
{
...
void ADungeonSpawner::SpawnTiles()
{
...
void ADungeonSpawner::SpawnWalls(bool IsFloorTile, UE::Math::TIntVector3<int> FloorTile, FVector SpawnLocation)
{
	for(int32 i =0 ; i<4;i++)
	{
		FRotator Rotation;
...
bool ADungeonSpawner::Room_FindNextLocation(FIntVector &NewLocation)
{
	bool IsFloorTile;
	bool Searching =true;
	bool Valid = false;
...
FIntVector ADungeonSpawner::TestRelativeTileLocation(FIntVector ReferenceLocation, int32 X,int32 Y ,bool &thisIsFloorTile )
{
...
TArray<FIntVector> ADungeonSpawner::Corridors_MapCorridors(FIntVector RoomA, FIntVector RoomB)
{
...
TArray<FIntVector> ADungeonSpawner::Corridors_MakeY(FIntVector From, FIntVector To)
{
...
TArray<FIntVector> ADungeonSpawner::Corridors_MakeX(FIntVector From, FIntVector To)
{
...
void ADungeonSpawner::NewMap()//generates a new map after the player has left through the end door 
{
